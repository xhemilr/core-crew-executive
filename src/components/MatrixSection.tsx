import { ButtonLink, Em, Reveal, Section, SectionHead, Wrap } from './ui/Primitives'
import { levels, matrix } from '../data/site'
    ...
    <Section id="solutions" tone="light">
      <Wrap>
        <SectionHead
          ...
          heading={
            <>
              Every trade. <Em tone="light">Every level.</Em>
            </>
          }
        ...
        <Reveal className="overflow-hidden rounded-[var(--radius-xl2)] border border-ivory-line bg-white shadow-[0_30px_70px_-50px_rgba(11,24,48,0.5)]">
          <table className="w-full table-fixed border-collapse">
            <caption className="sr-only">Worker profiles by sector and level of experience</caption>
            <thead className="hidden tab:table-header-group">
              <tr>
                ...
                >
                  <span className="block text-[13px] tracking-[0.14em] uppercase text-gold">Sector</span>
                  <span className="mt-1.5 block font-serif text-[22px] leading-tight">By level →</span>
                </th>
                {levels.map((level) => (
                  <th
                    key={level.name}
        ...
        <div className="mt-9 grid items-center gap-4 lg:grid-cols-[1fr_1fr_auto] lg:gap-10">
          <h3 className="font-serif text-[30px] leading-tight">
            Need a profile not listed here? <Em tone="light">Tell us.</Em>
          </h3>
          <p className="text-[15px] text-ink-muted">
            We match workers to the requirements of each assignment. Depending on the role, candidates
            may be reviewed for relevant experience, qualifications, skills, availability and
            documentation.
          </p>
