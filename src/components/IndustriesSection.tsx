import { Reveal, Rich, Section, SectionHead, Wrap } from './ui/Primitives'
import { industryImages } from '../data/site'
    ...
    <Section id="industries" tone="light">
      <Wrap>
        <SectionHead
          ...
          heading={<Rich text={t.industries.title} tone="light" />}
          text={t.industries.text}
        />

        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {t.industries.cards.map((card, index) => (
            <Reveal
              as="article"
              key={index}
              delay={index * 90}
        ...
        <div className="mt-12 grid items-start gap-6 border-t border-ivory-line pt-8 lg:grid-cols-[200px_1fr]">
          <h3 className="font-serif text-[26px] leading-tight">{t.industries.allSectors}</h3>
          <ul className="flex flex-wrap gap-2.5">
            {t.industries.chips.map((chip, index) => (
              <li
                key={index}
